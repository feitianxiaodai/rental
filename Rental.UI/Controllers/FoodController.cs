...
namespace Rental.UI.Controllers
{
    public class FoodController : Controller
    {
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult Create(Rental.Service.domain.FoodModel model, string[] ImgUrl)
        {
            if (ImgUrl != null && ImgUrl.Count() > 0)
            {
                foreach (var item in ImgUrl)
                {
                    model.FoodImageInfo.Add(new Rental.Service.domain.FoodImageInfoModel
                    {
                        ImgUrl = item
                    });
                }
        ...
        public ActionResult _AjaxDeleteFood(int[] ids)
        {
            ...
            bool result = foodSer.Delete(ids);
            if (result)
            {
        ...
        {
            var viewModel = foodSer.GetServiceById(id);
