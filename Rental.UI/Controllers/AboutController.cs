...
namespace Rental.UI.Controllers
{
    ...
    {
        Rental.Service.AboutService aboutSer = new Service.AboutService();
        public ActionResult Create()
        {
            var model = aboutSer.GetHotelAbout();
            return View(model);
        }
        ...
        public ActionResult _AjaxSaveAboutImg()
        {
            ...
            if (Directory.Exists(Server.MapPath("~/Upload/AboutImg")))
            {
                ...
                if (uploadFiles != null && uploadFiles.Count > 0)
                {
                    ...
                    {
                        url = string.Format("{0}Upload/AboutImg/{1}.{2}", hostName, actualFileName, fileExtension),
